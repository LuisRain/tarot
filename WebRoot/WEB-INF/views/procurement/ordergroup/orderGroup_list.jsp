               ...
               <option value ="3"  >3-创建人</option>  
					...
					<td>
						<span class="input-icon" id="searchinput">
							...
							<c:if test="${QX.edit == 1 }"><a class="btn btn-mini btn-light" onclick="goImportExcelPage();" title="导入批次订单">
							...
							<tr>
								...
								<td>
									<c:forEach items="${orderGroup.enos}" var="eno" varStatus="vsEN">
										<a onclick="getTeno('${eno.orderNum}');" id="tenNum">${vsEN.count}.${eno.orderNum }</a><br/>
									</c:forEach>
								</td>
								<td>
									<c:forEach items="${orderGroup.exos}" var="exo" varStatus="vsEX">
										<a onclick="getTexo('${exo.orderNum}');" id="texNum">${vsEX.count}.${exo.orderNum }</a><br/>
									</c:forEach>
								<td>
