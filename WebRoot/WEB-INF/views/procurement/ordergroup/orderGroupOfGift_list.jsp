  ...
  <div class="row-fluid">
	<div class="row-fluid">
               ...
               <option value ="3"  >3-创建人</option>  
					...
					<td>
						<span class="input-icon" id="searchinput">
							...
							<a class="btn btn-mini btn-light" onclick="goImportExcelPage();" title="导入批次赠品单">
					...
					<c:when test="${not empty varList}">
						<c:forEach items="${varList}" var="orderGroup" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${orderGroup.orderGroupId}</td>
								<td>${orderGroup.orderGroupNum }</td>
								<td>
		...
		function goImportExcelPage(){
			top.mainFrame.tabAddHandler("999999","导入批次赠品单","<%=basePath%>sellingorder/goImportExcelPageForGift.do");
		...
		function getTeno(tenoNumVal){
			top.mainFrame.tabAddHandler("32","赠品入库单","<%=basePath%>enwarehouseorder/list.do?searchcriteria=3&keyword="+tenoNumVal);
			if(MENU_URL != "druid/index.html"){
				jzts();
			}
		}
		function getTexo(texoNumVal){
			top.mainFrame.tabAddHandler("33","赠品出库单","<%=basePath%>exwarehouseorder/list.do?searchcriteria=3&keyword="+texoNumVal);
			if(MENU_URL != "druid/index.html"){
				jzts();
			}
		}
