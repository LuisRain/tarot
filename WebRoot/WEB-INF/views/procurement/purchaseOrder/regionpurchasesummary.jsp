...
<%@ include file="../../system/admin/top.jsp"%> 
	<script type="text/javascript">
		function fromExcel(){
			...
			window.location.href='purchaseOrder/regionpurchaseExport.do?searchcriteria='+searchcriteria+'&keyword='+keyword;
...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="purchaseOrder/regionpurchaseSum.do" method="post" id="PurchaseOrderForm">
			<table>
				<tr>
				<td>
			<select id="searchcriteria" name="searchcriteria" onchange="loaddate()">  
              <option value ="1"  >1-按批次号</option>
              <option value ="2"  >2-按供应商名称</option>
              <option value ="3"  >3-按供应商编码</option>
               </select>  
				</td>
					<td>
					...
					<c:when test="${not empty listpd}">
						<c:forEach items="${listpd}" var="list" varStatus="vs">
							...
							<td class='center' style="width: 30px;"><label><input
								...
								<td>${list.quantity+list.gift_quantity}</td>
								<%-- <td><fmt:formatNumber value="${list.purchase_price*(list.quantity+list.gift_quantity)}" pattern="##.##" minFractionDigits="2" ></fmt:formatNumber></td> --%>   
							</tr>
						</c:forEach>
					</c:when>
					<c:otherwise>
