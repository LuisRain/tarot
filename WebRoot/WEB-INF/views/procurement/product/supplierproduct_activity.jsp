...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="product/supplierproductActivity.do" method="post" id="ProductForm">
			<c:if test="${QX.cha == 1 }">
			<table>
				<tr><td>
					...
					<c:when test="${not empty varlist}">
						<c:forEach items="${varlist}" var="product" varStatus="vs">
							<tr>
								...
								<fmt:parseDate value="${product.begintime}" var="begintime" pattern="yyyy-MM-dd"></fmt:parseDate>
								<fmt:parseDate value="${product.endtime}" var="endtime" pattern="yyyy-MM-dd"></fmt:parseDate>
								...
								<td>
								<c:if test="${QX.edit == 1 }">
								<a class='btn btn-mini btn-info' title="编辑" onclick="editUser('${product.id }');"><i class='icon-edit'></i></a>
								</c:if>
								</td>
							</tr>
