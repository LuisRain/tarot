	...
	<form action="product/${msg }.do" name="Form" id="Form" method="post">
	   <input type="hidden" name="CARGO_SPACE_ID"  value="${product.tcsid}"/>
		...
		<tr>
			...
			<tr>
				<td class='center' >最小起订量:</td>
				<td ><input type="number" name="min_order_num" id="min_order_num" value="${product.min_order_num}"    maxlength="32" placeholder="最小起订量" title="最小起订量"/></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<select name="UNIT">
					...
					<option <c:if test="${list.id==product.unit}">selected="selected"</c:if>  value="${list.id}">${list.unit_name}</option>
					</c:forEach>
				</c:if>
			...
			<tr>
				<td>是否退货</td>
				<td>
				<label>可以<input type="radio" <c:if test="${product.is_th==2 }">checked="checked"</c:if>name="is_th" value="2"/>
				<span class="lbl"></span>
				</label>
				
				<label>不可以<input type="radio" <c:if test="${product.is_th==1 }">checked="checked"</c:if>  name="is_th" value="1"/>
				<span class="lbl"></span>
				</label>
				</td>
			</tr>
