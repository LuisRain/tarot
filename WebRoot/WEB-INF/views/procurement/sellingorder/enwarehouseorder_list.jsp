...
//导出excel
		function toExcel(){
			...
			window.location.href='<%=basePath%>sellingorder/dcexorderlistexcel.do?keyword='+keyword+'&searchcriteria='+searchcriteria+
			'&order_date='+orderdate+'&checked_state='+checked_state;
		}
//-->
</script>
</head>
<body>
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<!-- 检索  -->
					<form action="sellingorder/sellingRk.do" method="post" name="Form"
						id="Form">
						<c:if test="${QX.cha == 1 }">
						<table>
							<tr>
								...
								<span class="input-icon">
										<input class="span10 date-picker" id="orderdate"
										name="order_date" value="${pd.order_date}" type="text"
								...
								<td><select name="checked_state" id="checked_state">
										<option value="">状态过滤----请选择</option>
										<option value="1" <c:if test="${pd.checked_state==1}">selected="selected"</c:if>>1-待入库</option>
										<option value="2"<c:if test="${pd.checked_state==2}">selected="selected"</c:if>>2-已入库</option>
								</select></td>
								<td style="vertical-align:top;">
									<button class="btn btn-mini btn-light" onclick="search();"
										title="检索">
										<i id="nav-search-icon" class="icon-search"></i>
									</button> <c:if test="${pd.checked_state eq  1}">
									...
									<!-- <a class="btn btn-mini btn-light" onclick="fromExcel();" title="上传EXCEL"><i id="nav-search-icon" class="icon-cloud-upload"></i></a> -->
								</td>
							</tr>
						</table>
						</c:if>
										...
										<c:forEach items="${varList}" var="var" varStatus="vs">
											<tr>
												<td class='center' style="width: 30px;"><label><input
														...
														name='states' value="${var.checked_state}" /><span
									...
									<c:otherwise>
										<tr class="main_info">
											<c:if test="${pd.ivt_state eq  1}">
												<td colspan="100" class="center">没有待入库单</td>
											</c:if>
							...
							<table style="width:100%;">
								<tr>
									<td style="vertical-align:top;">
									 <c:if test="${QX.add == 1 }">
									<div>
									  <button  onclick="addenorder();" type="button" class="btn btn-small btn-success" title="新增入库单">
									  新增入库单</button></div>
								 	</c:if> 
									</td>
	...
	<script type="text/javascript">
		...
		//跳转新增页面
		function addenorder(){
			var url='<%=basePath%>sellingorder/goEnwareorderProduct.do';
				top.mainFrame.tabAddHandler("3920608252","查看入库单信息",url);
		}
		
		 //修改商品数量
		  function editProductNumber(id1,type){
			var url='<%=basePath%>sellingorder/goEnwareorderProductEdit.do?orderId='+id1+'&type='+type;
				top.mainFrame.tabAddHandler("3920608252","查看入库单信息",url);
	...
	<script type="text/javascript">
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要审核选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>sellingorder/sellingRkSh.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'text',
		...
		function fromExcel(){
			top.mainFrame.tabAddHandler("999999","导入入库订单","<%=basePath%>enwarehouseorder/goEnUploadExcel.do");
			if(MENU_URL != "druid/index.html"){
				jzts();
			}
		}
