...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="<%=basePath%>sellingorder/findsellingorderGrouplist.do" method="post" id="SellingOrderForm">
			<table>
				<tr>
				<td>
			<select id="searchcriteria" name="searchcriteria" onchange="loaddate()">
              <option value ="1"  >1-按门店名称/编码</option>
              <option value ="2"  >2-按采购日期</option>
             <!--  <option value="3" >3-按订单编号</option>   -->
              <option value="4" >3-按联系人</option>
               </select>
				</td>
					<td>
					...
					<c:when test="${not empty varList}">
						<c:forEach items="${varList}" var="purchaseOrder" varStatus="vs">
							<tr>
								...
								<td><a id="ordernum" onclick="findorderitem('${purchaseOrder.merchant_id }','${purchaseOrder.checked_state }','${purchaseOrder.type}');">${purchaseOrder.merchant_name}</a></td>
								...
								<td>
									<c:if test="${purchaseOrder.checked_state eq '12'}">新订单</c:if>
									<c:if test="${purchaseOrder.checked_state eq '1'}">地市已审核</c:if>
									<c:if test="${purchaseOrder.checked_state eq '2'}">省级已审核</c:if>
								</td>
								<td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		*	查询详情
		*/
		function findorderitem(id,state,type){

			location.href = '<%=basePath%>sellingorder/findorderitem_md.do?merchant_id='+id+'&state='+state+'&type='+type;
		}
		//审核操作
