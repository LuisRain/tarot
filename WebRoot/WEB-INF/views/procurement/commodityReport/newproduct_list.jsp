...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="product/newproductlist.do" method="post" id="ProductForm">
			<c:if test="${QX.cha == 1 }">
			<table>
				<tr><td>
					...
					<c:when test="${not empty productList}">
						...
						<c:forEach items="${productList}" var="product" varStatus="vs">
							...
							<tr>
								...
								<td>
								<c:choose>
								<c:when test="${QX.edit == 1 && pd.role_id ne 24}">
								<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${product.id}','${product.ROLE_ID}');"><i class='icon-edit'></i></a>
								</c:when>
								<c:otherwise>
									--
								</c:otherwise>
		...
		<table style="width:100%;">
			<tr>
				<c:if test="${pd.role_id eq 2 || pd.role_id eq 24}">
				
				<td style="vertical-align:top;">
					<a class="btn btn-small btn-success" onclick="siMenu('product9999','proudct999','新增商品','<%=basePath%>product/goAddNewProduct.do')">新增</a>						
...
//删除
		...
		function edit(Id,ROLE_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>product/gonewEdit.do?productId='+Id+'&ROLE_ID='+ROLE_ID;
			 diag.Width = 800;
			 diag.Height = 800;
