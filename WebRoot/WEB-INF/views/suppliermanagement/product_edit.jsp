   ...
   function setZtree(productType){
	...
	return false;
		...
		if($("#EXPIRE_DAYS").val()==""){
			...
			else if($("#EXPIRE_DAYS").val().length != 11 && !number.test($("#EXPIRE_DAYS").val())){
			$("#EXPIRE_DAYS").tips({
				side:3,
	            msg:'保质期只能填写数字',
	            bg:'#AE81FF',
		...
		else if($("#FCL_VOLUME").val().length != 11 && !xs.test($("#FCL_VOLUME").val())){
			$("#FCL_VOLUME").tips({
				side:3,
	            msg:'整体重量只能填写数字',
	            bg:'#AE81FF',
		...
		else if($("#FCL_WEIGHT").val().length != 11 && !xs.test($("#FCL_WEIGHT").val())){
			$("#FCL_WEIGHT").tips({
				side:3,
	            msg:'整体重量只能填写数字',
	            bg:'#AE81FF',
		...
		else if($("#box_number").val().length != 11 && !number.test($("#box_number").val())){
			$("#box_number").tips({
				side:3,
	            msg:'箱装数只能填写数字',
	            bg:'#AE81FF',
		...
		if($("#STORE_METHOD_CONSULTING_TELEPHONE").val()==""){
			$("#STORE_METHOD_CONSULTING_TELEPHONE").tips({
				side:3,
	            msg:'请输入服务电话',
	            bg:'#AE81FF',
	            time:2
	        });
			$("#STORE_METHOD_CONSULTING_TELEPHONE").focus();
			return false;
		}
		if($("#USAGE_MODE").val()==""){
			$("#USAGE_MODE").tips({
...
if($("#supplierid1").val()!=""&&$("#supplierid1").val()!=null){
	...
	else if($("#supplierprice1").val().length != 11 && !qian.test($("#supplierprice1").val())){
		$("#supplierprice1").tips({
			side:3,
	        msg:'金额输入格式不正确！',
	        bg:'#AE81FF',
	   ...
	   <input type="hidden" name="CARGO_SPACE_ID"  value="${product.tcsid}"/>
		...
		<tr>
			...
			<tr>
				...
				<c:otherwise>
				<td   colspan="2">
				<label>上架<input onclick="return false;" type="radio" checked="checked" name="IS_SHELVE" value="1"/>
				<span class="lbl"></span>
				</label>
				<label>下架<input onclick="return false;" type="radio" name="IS_SHELVE" value="2"/>
				<span class="lbl"></span>
				</label>
				</td>
				</c:otherwise>
			...
			<tr>
			  ...
			  <tr>
				...
				<td colspan="2" ><input type="text" disabled="disabled" id="sku_weight" name="sku_weight" value="${product.sku_weight}" maxlength="32" placeholder="单品重量" title="单品重量"/></td>
			</tr>
			
			
			<tr>
				<td class='center' >单品体积:</td>
				<td colspan="2" ><input type="text" disabled="disabled"  name="sku_volume" id="sku_volume" value="${product.sku_volume}" maxlength="32" placeholder="这里输入单品体积" title="单品体积"/></td>
			</tr>
			<tr>
			...
			<!--  <tr>
				...
				<td ><input type="text" name="TAXES" id="TAXES" value="${product.taxes}" maxlength="32" placeholder="这里输入税率" title="税率"/></td>
			</tr>
			<tr>
				<td class='center' >重量单位:</td>
				<td ><input type="text" name="WEIGHT_UNIT" id="WEIGHT_UNIT" value="${product.weight_unit}" maxlength="32" placeholder="这里输入重量单位" title="重量单位"/></td>
			</tr>
			<tr>
				<td class='center' >储藏方式:</td>
				<td ><input type="text" name="STORE_METHOD" id="STORE_METHOD" value="${product.store_method}" maxlength="32" placeholder="这里输入储藏方式" title="储藏方式"/></td>
			</tr>
			<tr>
				<td class='center' >服务电话:</td>
				<td ><input type="text" name="STORE_METHOD_CONSULTING_TELEPHONE" id="STORE_METHOD_CONSULTING_TELEPHONE" value="${product.store_method_consulting_telephone}" maxlength="32" placeholder="这里输入服务电话" title="服务电话"/></td>
			</tr>
			<tr>
				<td class='center' >使用方式:</td>
				<td ><input type="text" name="USAGE_MODE" id="USAGE_MODE" value="${product.usage_mode}" maxlength="32" placeholder="这里输入使用方式" title="使用方式"/></td>
			</tr>
			<tr>
				<td class='center' >产地:</td>
				<td ><input type="text" name="ORIGIN_PLACE" id="ORIGIN_PLACE" value="${product.origin_place}" maxlength="32" placeholder="这里输入产地" title="产地"/></td>
			</tr>-->
