...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="bulk/list.do" method="post" id="bulkForm">
			<c:if test="${QX.cha == 1 }">
			<table>
				<tr>
				<td>
              ...
              <option value="8" >第八巷道</option>
				...
				<input autocomplete="off" id="nav-search-input" type="text" name="keyword" value="${pd.keyword}" placeholder="这里输入关键词" />
					...
					<td style="vertical-align:top;">
									...
									<a class="btn btn-mini btn-light" onclick="beifen();" title="备份分拣信息">
										备份分拣信息
									</a>
								</td>
				</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>exwarehouseorder/excelOfMerchant.do';
		}
		//导出excel
		function beifen(){
			$.post("<%=basePath%>bulk/insertsqlserver.do",function(data){
				if(data=="success"){
					alert("备份数据成功");
				}else{
					alert("备份数据失败");
