  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<select id="searchcriteria" name="searchcriteria">  
              <option value ="1"  >1-按仓库名称</option>  
              <option value ="2"  >2-按仓库地址</option>  
              <option value="3" >3-按联系人电话</option>  
              <option value="4" >4-按仓库编号</option>  
               </select>  
				</td>
					<td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										...
										<td>${var.warehouse_tel}</td>
										<td>${var.warehouse_number}</td>
										<td>
										<c:if test="${var.warehouse_type ==1}">一级仓库</c:if>
										<c:if test="${var.warehouse_type ==2}">二级仓库</c:if>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>warehouse/goEdit.do?id='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
