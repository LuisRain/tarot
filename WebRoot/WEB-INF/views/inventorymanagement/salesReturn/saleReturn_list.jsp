...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	...
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="<%=basePath%>exwarehouseorder/purchaseReturnlistpage.do" method="post" name="Form" id="Form">
			<c:if test="${QX.cha == 1 }">
			<table>
				<tr>
						<td>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
									<td><a onclick="editProductNumber('${var.order_num}',2)">${var.order_num}</a></td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function search(){
			...
			function add(){
				...
				var url='<%=basePath%>exwarehouseorder/addPurchaseOrder.do';
				top.mainFrame.tabAddHandler("3920638356","添加销售退货单",url);
			}
		//删除
		function del(Id){
	  ...
	  function editProductNumber(id1,type){
		var url='<%=basePath%>exwarehouseorder/goEnwareorderPurchaseProductEdit.do?orderId='+id1+'&type='+type;
			top.mainFrame.tabAddHandler("39206033322235","查看销售退货信息",url);
	 } 
	//批量操作
	//@ sourceURL=ccccc.js
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要审核选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>exwarehouseorder/saleReviewedAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
