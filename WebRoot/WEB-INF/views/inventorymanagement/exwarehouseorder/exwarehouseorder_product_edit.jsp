...
<link href="<%=basePath%>static/css/bootstrap.min.css" rel="stylesheet" />
	...
	<script type="text/javascript" src="<%=basePath%>static/js/jQuery.print.js"></script>
		...
		function print(){
			window.open("<%=basePath%>exwarehouseorder/print.do?orderId=${enwarhouse.order_num}&type=1", "", 'left=250,top=150,width=1400,height=500,toolbar=no,menubar=no,status=no,scrollbars=yes,resizable=yes');	
		}
		</script>
<body>
		...
		<tr>
			...
			<tr>
				...
				<td>
				  ...
				  <td style="width:120px;text-align: right;padding-top: 13px;">7便利店联系人:</td>
				<td>
				...
				<c:choose>
					<c:when test="${not empty orderItemList}">
						<c:forEach items="${orderItemList}" var="o" varStatus="vs">
							...
							type="number" value="${o.purchase_price}" minFractionDigits="2" maxFractionDigits="4" /></td>
							      <td class="center"><fmt:formatNumber type="number" value="${o.quantity}" maxFractionDigits="0"/> </td>
								<c:if test="${type eq  1}">
													<td class="center">
														<c:if test="${enwarhouse.ivt_state eq 11}">
														...
														<c:if test="${enwarhouse.ivt_state  ne    11 }">
														 <p id="p${o.id}">
				 ...
				 <c:if test="${enwarhouse.ivt_state eq 1}">
								<tr id="noprint">
									<td colspan="5"  > 选择商品: <input type="text" name="productId" id="enProductId"
