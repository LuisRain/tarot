...
<link href="<%=basePath%>static/css/bootstrap.min.css" rel="stylesheet" />
	...
	<script type="text/javascript" src="<%=basePath%>static/js/jQuery.print.js"></script>
		...
		function print(){
			window.open("<%=basePath%>exwarehouseorder/printpq.do?orderId=${enwarhouse.order_num}&type=1", "", 'left=250,top=150,width=1400,height=500,toolbar=no,menubar=no,status=no,scrollbars=yes,resizable=yes');	
		}
		</script>
<body>
...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			<form action="exwarehouseorder/cpgoExwareorderProductEditPQ.do?orderId=${enwarhouse.order_num}	" method="post" name="Form" id="Form">
		...
		<tr>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<c:choose>
					<c:when test="${not empty orderItemList}">
						<c:forEach items="${orderItemList}" var="o" varStatus="vs">
							...
							type="number" value="${o.sale_price}" minFractionDigits="2" maxFractionDigits="4" /></td> --%>
							      <td class="center">
							      <fmt:formatNumber type="number" value="${o.quantity}" maxFractionDigits="0"/> (${o.unit_name}) 
							      </td>
								<td class="center">
									<c:if test="${enwarhouse.ivt_state eq 1 }">
										<input name="productNumber" type="text" style="width:50px;"
											value='<fmt:formatNumber type="number" value="${o.final_quantity}"  maxFractionDigits="0"/> '  id="input${o.id}"> (${o.unit_name})
									</c:if> 
									<c:if test="${enwarhouse.ivt_state  ne 1 }">
										<p id="p${o.id}">
										<fmt:formatNumber type="number" value="${o.final_quantity}" maxFractionDigits="0"/> (${o.unit_name}) </p>
									</c:if>
									<input style="display: none" type="text" name="orderItemId" VALUE="${o.id}">
								</td>
											
												<td class='center'>${o.gift_quantity }(${o.unit_name})</td>
											<%--  <td class="center"><fmt:formatNumber type="number" value="${o.price}" minFractionDigits="2" maxFractionDigits="4" /></td> --%>
											 <td class="center">
												 <c:if test="${not empty o.total_count}">
												 	 <fmt:formatNumber type="number" value="${o.total_count}" maxFractionDigits="0"/>(箱)
												 </c:if>
												 <c:if test="${empty o.total_count}">
												 	--
												 </c:if>
											 </td>
											 <td class="center">
												 <c:if test="${not empty o.per_count}">
												 <fmt:formatNumber type="number" value="${o.per_count}" maxFractionDigits="0"/>(${o.unit_name})
												 </c:if>
												 <c:if test="${empty o.per_count}">
												 	--
												 </c:if>
											 </td>
										 <td>${o.comment}</td>	
				 ...
				 <%-- <c:if test="${enwarhouse.ivt_state eq 1}">
								<tr id="noprint">
									<td colspan="8"  > 选择商品: <input type="text" name="productId" id="enProductId"
			        ...
			        <td colspan="4" id="finalAmount" class='center'><font style="color: red;font-weight:bold;">
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				合&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;计：
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				</font> 
				...
				<td></td>
				<%-- <td class="center"><font style="color: red;font-weight:bold;"><!-- 出库单总额: --><fmt:formatNumber
							type="number" value="${enwarhouse.final_amount}" minFractionDigits="2" maxFractionDigits="2" />元</font></td> --%>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function search(){
		  ...
		  function editProductNumber(id1){
			var url='<%=basePath%>exwarehouseorder/goExwareorderProductEdit.do?type=1&orderId='+id1;
