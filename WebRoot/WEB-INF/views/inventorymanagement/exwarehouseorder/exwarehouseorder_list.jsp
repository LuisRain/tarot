...
//导出excel
		function toExcelgg(){
            ...
            if(chestr==""){
				...
				window.location.href='<%=basePath%>exwarehouseorder/dcexorderlistexcel.do?keyword='+keyword+'&searchcriteria='+searchcriteria+
...
<body>
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<form action="exwarehouseorder/list.do" method="post" name="Form"
						id="Form">
						...
						<table>
							...
							<input class="span10 date-picker" id="orderdate" name="order_date" value="${pd.order_date}"
							 type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:110px; display: none;" 
							 placeholder="按订购日期" title="按订购日期" />
							 <input autocomplete="off" 	id="nav-search-input" type="text" name="keyword" 	value="${pd.keyword}" placeholder="这里输入关键词" />
											<select name="ivt_state" id="ivt_state">
												...
												<option value="1">0-未分配订单</option>
												<option value="11">1-新出库单</option>
												<option value="2">2-LED系统正在分拣</option>
												<option value="3">3-分拣完成</option>
												<option value="4">4-订单已合并</option>
												<option value="5">5-订单已出库</option>
												<option value="6">6-完成</option>
											</select>
											
											<select name="ivt_type" id="ivt_type">
												<option value="-1">运输状态过滤</option>
												<option value="" <c:if test="${empty pd.ivt_type}">selected="selected"</c:if>>0-未添加计划</option>
												<option value="0" <c:if test="${pd.ivt_type==0 }">selected="selected"</c:if>>1-未发布</option>
												<option value="1" <c:if test="${pd.ivt_type==1 }">selected="selected"</c:if>>2-未装车</option>
												<option value="2" <c:if test="${pd.ivt_type==2 }">selected="selected"</c:if>>3-运输中</option>
												<option value="3" <c:if test="${pd.ivt_type==3 }">selected="selected"</c:if>>4-已签收</option>
											</select>
										<c:if test="${pd.ivt_state eq 11}">
											<a style="margin-bottom: 10px" class="btn btn-warning btn-mini"
												onclick="showAndEditWaveForExOrder('确定要波次操作您所选的出库单吗?');"
												title="出库单波次分拣"> <i id="nav-search-icon" class='icon-wrench icon-2x icon-only'></i>
											</a>
											<!-- <a class="btn btn-mini btn-light" onclick="toSplitEXToExcel();" title="导出商品拆分出库单">
									<i id="nav-search-icon" class="icon-download-alt"></i>
								</a> -->
										</c:if> <c:if test="${pd.ivt_state eq 2}">
											<a style="margin-bottom: 10px" class="btn btn-warning btn-mini"
												onclick="toWaveForExOrder('确定要波次操作您所选的出库单吗?');"
												title="管理波次出库单"><i id="nav-search-icon" class='icon-wrench icon-2x icon-only'></i>
											</a>
										...
										</c:if> <c:if test="${pd.ivt_state eq 4}">
							...
							<c:if test="${pd.ivt_state eq 1}">
									<button type="button" class="btn btn-mini btn-light" style="margin-bottom: 10px" onclick="jssl();" title="自动分配">自动分配</button>
							</c:if>
									<!-- <button type="button" class="btn btn-mini btn-light" style="margin-bottom: 10px" onclick="toExcelgg();" title="导出到EXCEL">
									<i id="nav-search-icon" class="icon-download-alt"></i> </button> -->
								</td>
							</tr>
						</table></c:if>
						<c:if test="${pd.ivt_state != 1}">
									...
									<c:when test="${not empty varList}">
										<c:forEach items="${varList}" var="var" varStatus="vs">
											<tr>
												...
												<td class="center">
												<a style="color: #08c" ; onclick="editProductNumber('${var.order_num}',1)">${var.order_num}</a>
												&nbsp;&nbsp;&nbsp;&nbsp;
												<a style="color: #08c" ; onclick="editProductNumberPQ('${var.order_num}',1)">分拣详单</a>
												</td>
													<td class="center">
												...
												<td class="center">${var.total_weight}</td> 
													...
													<td>
													<c:if test="${empty var.type}">未添加计划单号</c:if>
	...
	<script type="text/javascript">
		function jssl(){
			bootbox.confirm("确定要自动分配吗?", function(result) {
				if(result) {
					top.jzts();
					var url="exwarehouseorder/jsxg.do";
					$.post(url,function(data){
						alert(data);
						nextPage(${page.currentPage});
						if(data=="成功！"){
							var url='<%=basePath%>exwarehouseorder/Exconfirmed.do';
							top.mainFrame.tabAddHandler("97976610","查看出库单分量详情",url);
						}
					})
				}
			});
		...
		function SelectNo() {
		  ...
		  function editProductNumber(id1,type){
			var url='<%=basePath%>exwarehouseorder/goExwareorderProductEdit.do?orderId='+id1+'&type=1';
					...
					for(var i=0;i < document.getElementsByName('ids').length;i++){
						  if(document.getElementsByName('ids')[i].checked){
						  	if(str=='') str += document.getElementsByName('ids')[i].value;
						  	  ...
						  	  if(document.getElementsByName("states")[i].value!=11){
