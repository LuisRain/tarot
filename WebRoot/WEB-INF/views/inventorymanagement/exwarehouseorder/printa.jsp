...
<script type="text/javascript" src="static/js/twomsswms_print.js"></script>
 <style type="text/css">
		.clear{clear: both;}
		body,td,th{font: 12px/1.5 "Microsoft yahei",SimHei normal;font-family: "黑体"}
		...
		.print-title h2{font-size:25px!important;font-family: "黑体"}
		.print-title h4{font-size:20px!important;font-family: "黑体"}
		.signatureArea {
			height: 25px;
			line-height: 25px;
		}
 ...
 <body>
	...
	<c:forEach items="${listpd}" var="va" varStatus="vs">
	<div class="print-title" id="headerInfo">
		<h4>中石油山西昆仑好客便利店</h4>
		<h2>中央仓配送商品验收单</h2>
		<span class="floatLeft"> 门店：  ${va.merchant_id}| ${va.merchant_name}|${va.short_name}<br/>门店地址：${va.address}</span>
	...
	<thead>
		...
		<c:choose>
					<c:when test="${not empty orderItemList}">
						<c:forEach items="${orderItemList}" var="o">
							<tr>
							 ...
							 <td>${o.bar_code}</td>
							 <td>${o.unit_name}</td>
							 <td><fmt:formatNumber type="number" value="${o.final_quantity}" maxFractionDigits="0" /></td>
							  <td><fmt:formatNumber type="number" value="${o.gift_quantity}" maxFractionDigits="0" /></td>
				...
				<fmt:formatNumber type="number" value="${va.final_quantity}" maxFractionDigits="0" />
				</font> 
				</td>
				<td class="center">
				<font style="color: red;font-weight:bold;"><!-- 赠品总数: -->
				<fmt:formatNumber type="number" value="${va.gift_quantity}" maxFractionDigits="0" />
				</font>
				</td>
				<td class="center">
				...
				<td colspan="5" style="text-align: left;">门店收货： </td>
				</tr>
				</tfoot>
	</table>
	</table>
