				...
				<div class="fr1">
					<select id="searchcriteria" name="searchcriteria"
									onchange="loaddate()">
										...
										<option value="1">1-加急订单</option>
										<option value="2">2-仓配订单</option>
								</select>
					<input id="groupnum" name="groupnum" style="width: 200px" type="text" value="" placeholder="输入格式如：20180601" >
					<button type="button" class="btn btn-mini btn-light" style="margin-bottom: 10px" onclick="xsexprot();" title="导出到EXCEL">
		...
		$(top.hangge());
			function exprot(){
				...
				if(groupnum!=null && groupnum!=""){
					window.location.href='<%=basePath%>exwarehouseorder/exportexcel.do?groupnum='+groupnum+'&searchcriteria='+searchcriteria;
			...
			function xsexprot(){
				...
				var searchcriteria=$("#searchcriteria").val();
				if(groupnum!=null && groupnum!=""){
					window.location.href='<%=basePath%>exwarehouseorder/xsportexcel.do?groupnum='+groupnum+'&searchcriteria='+searchcriteria;
					layer.alert("导出成功",{icon:1});
				}else{
					layer.alert('请输入批次', {icon: 2});   
				}
