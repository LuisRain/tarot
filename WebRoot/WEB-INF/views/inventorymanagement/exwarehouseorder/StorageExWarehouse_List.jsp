...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="exwarehouseorder/StorageExWarehouseList.do" method="post" name="Form" id="Form">
			<table>
				<tr>
						<td>
              ...
              <option value="4" >4-按批次号</option>    
					...
					<option value="2">2-已出库</option>
		               ...
		               <c:if test="${pd.ivt_state eq  1}">
									...
									<a class="btn btn-mini btn-light" onclick="fromExcel();" title="从EXCEL导入"><i id="nav-search-icon" class="icon-cloud-upload"></i></a>
					</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function del(Id){
		 ...
		 //修改商品数量
		  function editProductNumber(id1,type){
			var url='<%=basePath%>exwarehouseorder/goStorageExwareorderProductEdit.do?orderId='+id1+'&type='+type;
				top.mainFrame.tabAddHandler("392060852","查看出库单信息",url);
		...
		//修改
		function add(){
			var url='<%=basePath%>exwarehouseorder/addStorageoBeputinstorage.do';
			top.mainFrame.tabAddHandler("3920608457","添加单个出库单",url);
		...
		//导出excel
		function toExcel(id){
			window.location.href='<%=basePath%>exwarehouseorder/excel.do?id='+id;
		}
		...
		function fromExcel(){
			 ...
			 diag.URL = '<%=basePath%>exwarehouseorder/goUploadExcel.do';
			 diag.Width = 300;
			 diag.Height = 150;
