		...
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<!-- 检索  -->
					<c:if test="${QX.cha == 1 }">
					<form action="thproduct/XsOrderList.do" method="post" name="Form"
						id="Form">
						...
						<table>
							<tr>
								<td><select id="searchcriteria" name="searchcriteria"
									onchange="loaddate()">
										<option value="1">1-按客户名称</option>
										<option value="2">2-按订购日期</option>
										<option value="3">3-按退货单编号</option>
								...
								<c:choose>
									<c:when test="${not empty varList}">
											...
											<tr>
												<td class='center' style="width: 30px;">
													...
													<label style="display: none">
														<input name='states' value="${var.ivt_state}" />
														<span class="lbl"></span>
													</label>
												</td>
												...
												<td>${var.group_num}</td>
												<td class='center'><a
													onclick="editProductNumber('${var.order_num}')">${var.order_num}</a></td>
												<td class='center'><c:if test="${var.checked_state eq  1 and var.order_type eq  2}">
												 待入库
												</c:if> <c:if test="${var.checked_state eq  2 and var.order_type eq  2}">
												  已入库
												</c:if></td>
										...
										<td class='center' >${var.total_weight}</td> --%>
												...
												<td class='center'>${fn:substring(var.create_time,0,10)}</td>
												<%-- <td>
												<c:if test="${QX.cha == 1 }">
		...
		function del(Id){
		 ...
		 //修改商品数量
		  function editProductNumber(id1){
			var url='<%=basePath%>thproduct/goEnwareorderProductEdit.do?orderId='+id1;
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要审核选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>thproduct/xsExamine.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'text',
