...
<body>
	<form action="user/${msg }.do" name="userForm" id="userForm" method="post">
		...
		<table>
			...
			<tr>
	<td>
		       <select class="chzn-select" name="ck_id" id="ck_id" data-placeholder="请选择所属仓库" style="vertical-align:top;">
		       	<option value="0">所有仓库</option>
				<c:forEach items="${warehouse}" var="warehouse">
					<option value="${warehouse.id }" <c:if test="${warehouse.id == pd.ck_id }">selected</c:if>>${warehouse.name }&nbsp;&nbsp;&nbsp;&nbsp;---&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${warehouse.level_id}级仓库</option>
				</c:forEach>
		        </select>
				</td>
				</tr>
